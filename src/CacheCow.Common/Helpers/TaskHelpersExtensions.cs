// Copied from ASP.NET Web API Source https://git01.codeplex.com/aspnetwebstack.git
...
namespace CacheCow.Common
{
	public static class TaskHelpersExtensions
	{
			...
			if (task.IsCompleted)
			{
				if (task.IsFaulted)
				{
					try
					{
						...
						if (resultTask == null)
						{
							// Not a resource because this is an public class, and this is a guard clause that's intended
		...
		/// </summary>
		public static Task CopyResultToCompletionSource<TResult>(this Task task, TaskCompletionSource<TResult> tcs, TResult completionResult)
		{
		...
		/// </summary>
		[SuppressMessage("Microsoft.WebAPI", "CR4001:DoNotCallProblematicMethodsOnTask", Justification = "The usages here are deemed safe, and provide the implementations that this rule relies upon.")]
		public static Task CopyResultToCompletionSource<TResult>(this Task<TResult> task, TaskCompletionSource<TResult> tcs)
		{
		...
		/// </summary>
		[SuppressMessage("Microsoft.Design", "CA1031:DoNotCatchGeneralExceptionTypes", Justification = "The caught exception type is reflected into a faulted task.")]
		public static Task<TResult> Finally<TResult>(this Task<TResult> task, Action continuation, bool runSynchronously = false)
		{
			// Stay on the same thread if we can
			if (task.IsCompleted)
			{
			...
			else
			{
				...
				try
				{
					if (AppDomain.CurrentDomain.IsFullyTrusted)
					{
						...
						// This is an public method in mscorlib.dll, so pass a test Exception to it to make sure we can call it.
